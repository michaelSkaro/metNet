...
# make a function to filter the headers to only the features that were migrated from the 450k array to the 850k
migrated_cgIDs_list = pd.read_csv("/home/jovyan/storage/Machine_Learning/infinium-methylationepic-vs-humanmethylation450.csv")
...
migrated_cgIDs = collect_migrated_cgIDs(migrated_cgIDs_list)
...
def read_cut(file, start, end, migrated_cgIDs):
...
df.to_csv("Selected_features_all_methylation_data_TCGA_2.csv")
...
X = pd.read_csv("Selected_features_all_methylation_data_TCGA_2.csv")
